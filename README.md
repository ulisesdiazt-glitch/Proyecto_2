Buenas Equipo!!!

Recordad:

- Victor Fernando - Técinco JS (Interactivo): 
  · Descargar del repositorio el archivo 'Proyecto.JS'.
  · Abrirlo en Visual Studio.
  · Copiar el contenido del archivo que te he enviado por email y pegarlo en el 'Proyecto.js'.
  · Volver a subirlo.

- Víctor Rey - Técnico CSS (Estilista):
  · Descarga el archivo del repo llamado 'Proyecto.css'.
  · Haz una hoja de estilos sencillita con un fondo del color que quieras.
  · Si quieres darle estilo a algo más, hay una etiqueta \<h1>\, un \<p>\ y 2 botones, uno con id 'toeuro' y otro 'todollar'.


  POR LO DEMÁS, RECORDAD NO CAMBIAR EL NOMBRE DEL ARCHIVO PORQUE SI NO, NO VA A FUNCIONAR CUANDO LOS JUNTE.

  SI POR LO QUE SEA LO HACÉIS, AVISADME PARA CAMBIAR LOS HREFS EN MI HTML PARA QUE APUNTE HACIA LOS ARCHIVOS RENOMBRADOS.


  Víctor Fernando, realizado commit y push
                
